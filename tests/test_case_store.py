...
def store(tmp_path, user):
    store = CaseStore(tmp_path / "resolve.db")
    store._test_user_id = user["id"]
    return store
...
def test_one_active_case_per_session(store):
    store.create_case("session-abc", user_id=store._test_user_id, title="A", category="warranty", description="D")
    with pytest.raises(ValueError, match="already exists"):
        store.create_case("session-abc", user_id=store._test_user_id, title="B", category="warranty", description="D")


def test_different_sessions_get_separate_cases(store):
    first = store.create_case("s1", user_id=store._test_user_id, title="A", category="warranty", description="D")
    second = store.create_case("s2", user_id=store._test_user_id, title="B", category="refund", description="D")
    assert first["id"] != second["id"]
    assert store.get_case_by_session("s1")["id"] == first["id"]


def test_update_case_fields(store):
    case_id = store.create_case("s1", user_id=store._test_user_id, title="A", category="warranty", description="D")["id"]
...
def test_update_case_amount_coerced_to_float(store):
    case_id = store.create_case("s1", user_id=store._test_user_id, title="A", category="refund", description="D")["id"]
...
def test_restart_persistence(tmp_path, user):
    ...
    case_id = store_one.create_case("s1", user_id=user["id"], title="A", category="warranty", description="D")["id"]
...
def test_list_cases(store):
    store.create_case("s1", user_id=store._test_user_id, title="A", category="warranty", description="D")
    store.create_case("s2", user_id=store._test_user_id, title="B", category="refund", description="D")
...
def test_transition_keeps_unrelated_case_fields(store):
    case_id = store.create_case(
        "s1", user_id=store._test_user_id, title="A", category="warranty", description="D"
    )["id"]
    store.update_case(
        case_id,
...
def test_transition_to_resolved_clears_next_action(store):
    """A resolved case must not retain a stale pending-user-action instruction."""
    case_id = store.create_case(
        "s1", user_id=store._test_user_id, title="A", category="warranty", description="D"
    )["id"]
    store.update_case(
